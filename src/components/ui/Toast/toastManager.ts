...
class ToastService {
  ...
  getContainer(): HTMLDivElement | null {
    ...
    if (!container) {
      ...
      container.className = `inset-auto m-0 bg-transparent overflow-visible border-0 ${toastConfig.position} z-[9999] w-[356px] max-w-[calc(100vw-2rem)] flex flex-col items-end pointer-events-none p-4 group perspective-1000`;
  ...
  show(
    message: string,
    type: ToastType = "info",
    duration = toastConfig.autoCloseDelay,
  ) {
    ...
    const alertElement = React.createElement(
      ...
      [
        React.createElement(
          AlertTitle as any,
          { key: "title" },
          toastConfig.titles[type],
        ),
        React.createElement(
          AlertDescription as any,
          { key: "description" },
          message,
        ),
      ],
  ...
  destroyContainer() {
    ...
    if (container) {
      ...
      if (typeof (container as any).hidePopover === "function") {
        try {
  ...
  close(toastId: string) {
    ...
    window.setTimeout(() => {
      ...
      if (this.toasts.length === 0 && this.queue.length === 0) {
